  ...
  return (
    <div className={`bg-white dark:bg-slate-800/90 rounded-3xl shadow-card border border-gray-100 dark:border-slate-700/80 p-5 space-y-6 ${className}`}>
      ...
      <div className="flex items-center justify-between pb-3 border-b border-gray-100 dark:border-slate-700/60">
        <span className="font-display font-semibold text-sm text-gray-900 dark:text-white">Filters</span>
        {isFiltered && onReset && (
          <button
            onClick={onReset}
      ...
      <div>
        <div className="flex items-center justify-between mb-2">
          <p className="text-xs font-bold text-gray-500 dark:text-slate-400 uppercase tracking-wider">Max Monthly Budget</p>
      ...
      <div className="flex items-center justify-between py-1">
        <p className="text-xs sm:text-sm font-medium text-gray-800 dark:text-slate-200">AC Rooms Only</p>
        <button
          type="button"
          onClick={() => onChange("acOnly", !filters.acOnly)}
          className={`relative w-11 h-6 rounded-full transition-colors cursor-pointer ${
            filters.acOnly ? "bg-[#2A556A] dark:bg-sky-500" : "bg-gray-200 dark:bg-slate-700"
          }`}
        >
          <span
      ...
      <div className="space-y-2.5 pt-1 border-t border-gray-100 dark:border-slate-700/60">
        <p className="text-xs font-bold text-gray-500 dark:text-slate-400 uppercase tracking-wider">Meals &amp; Food</p>
        <label className="flex items-center gap-2.5 cursor-pointer select-none">
          ...
          />
          <span className="text-xs sm:text-sm text-gray-700 dark:text-slate-200 font-medium">Mess Included (3 Meals)</span>
        </label>
        <label className="flex items-center gap-2.5 cursor-pointer select-none">
          <input
            type="checkbox"
            ...
            className="w-4 h-4 rounded border-gray-300 dark:border-slate-600 accent-[#2A556A] dark:accent-sky-400 cursor-pointer"
          />
          <span className="text-xs sm:text-sm text-gray-700 dark:text-slate-200 font-medium">Tiffin Delivery Available</span>
        </label>
      </div>

      {/* Rules & Policies (Curfew & Visitors) */}
      <div className="space-y-2.5 pt-1 border-t border-gray-100 dark:border-slate-700/60">
        <p className="text-xs font-bold text-gray-500 dark:text-slate-400 uppercase tracking-wider">Rules &amp; Freedom</p>
        <div className="flex items-center justify-between">
          <span className="text-xs sm:text-sm text-gray-700 dark:text-slate-200 font-medium">No Curfew Restriction</span>
          <button
            type="button"
            onClick={() => onChange("noCurfew", !filters.noCurfew)}
            className={`relative w-11 h-6 rounded-full transition-colors cursor-pointer ${
              filters.noCurfew ? "bg-[#2A556A] dark:bg-sky-500" : "bg-gray-200 dark:bg-slate-700"
            }`}
          >
            <span
              className={`absolute top-0.5 left-0.5 w-5 h-5 bg-white rounded-full shadow-xs transition-transform ${
        ...
        <div className="flex items-center justify-between">
          ...
          <button
            ...
            className={`relative w-11 h-6 rounded-full transition-colors cursor-pointer ${
              filters.visitorAllowed ? "bg-[#2A556A]" : "bg-gray-200"
            }`}
          >
            <span
              className={`absolute top-0.5 left-0.5 w-5 h-5 bg-white rounded-full shadow-xs transition-transform ${
