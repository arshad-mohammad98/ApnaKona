...
export default function ThemeToggle({ variant = "icon", className = "" }: ThemeToggleProps) {
  ...
  if (!mounted) {
    return (
      <div className={`w-11 h-11 min-h-[44px] min-w-[44px] rounded-xl bg-gray-100 dark:bg-slate-800 animate-pulse ${className}`} />
    );
  }
  ...
  // Default toggle button (icon variant styled as matching block)
  return (
    <button
      type="button"
      onClick={toggleTheme}
      ...
      className={`group relative flex items-center justify-center min-h-[44px] min-w-[44px] px-2.5 py-2 rounded-xl text-white bg-[#F4A261] hover:bg-[#e7924e] shadow-sm shadow-[#F4A261]/25 transition-colors cursor-pointer border-0 ${className}`}
    >
      {resolvedTheme === "dark" ? (
        <Sun className="w-5 h-5 text-white rotate-0 transition-transform group-hover:rotate-45 stroke-[2.5]" />
      ) : (
        <Moon className="w-5 h-5 text-white rotate-0 transition-transform group-hover:-rotate-12 stroke-[2.5]" />
      )}
    </button>
  );
}
