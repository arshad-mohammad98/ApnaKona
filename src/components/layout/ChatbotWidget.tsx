...
const FALLBACK_KNOWLEDGE: Record<string, string> = {
  ...
  "What is a realistic budget range for student PGs and hostels?":
    ...
    "- **Greater Noida (Knowledge Park, Pari Chowk, Alpha 1)**: ₹6,500 – ₹13,000/mo (Near Sharda, Galgotias, Bennett)\n" +
  ...
  return (
    <div>
      ...
      >
        ...
        {/* Quick Replies Panel */}
        {isInitialState && (
          <div className="px-4 py-2 border-t border-gray-100 dark:border-slate-700/60 bg-white/95 dark:bg-slate-900/95 shrink-0">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-1.5">
              {QUICK_REPLIES.map(({ label, icon: Icon, prompt }) => (
                <button
                  ...
                  className="flex items-center gap-2 p-2.5 text-xs text-left text-gray-800 dark:text-slate-200 bg-gray-50 dark:bg-slate-800 hover:bg-[#0F4C81]/10 dark:hover:bg-sky-500/20 hover:text-[#0F4C81] dark:hover:text-sky-300 rounded-xl border border-gray-200/60 dark:border-slate-700 transition-colors cursor-pointer min-h-[44px]"
                >
                  <Icon className="w-3.5 h-3.5 text-[#0F4C81] dark:text-sky-400 shrink-0" />
