...
export default function Navbar() {
  ...
  const navLinks = [
    { href: "/hostels", label: t("nav.findHostels", "Find Hostels"), icon: Building2 },
    { href: "/explore", label: t("nav.exploreHubs", "Campus Hubs"), icon: Compass },
    { href: "/connect", label: t("nav.roomieConnect", "Roommates"), icon: Users },
    { href: "/grievance", label: t("nav.grievance", "Student Grievance"), icon: AlertCircle },
  ];
        ...
        <div className="flex items-center justify-between h-16">
          ...
          <div className="hidden md:flex items-center gap-2 lg:gap-3">
            ...
            <button
              ...
              className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-xl border border-gray-200/80 dark:border-slate-700 bg-white/70 dark:bg-slate-800/80 hover:border-[#0F4C81] dark:hover:border-sky-400 text-xs font-semibold text-gray-700 dark:text-slate-200 transition-all cursor-pointer min-h-[38px]"
            >
              <Globe className="w-3.5 h-3.5 text-[#0F4C81] dark:text-sky-400" />
          ...
          {/* Mobile Controls (Language + Theme Toggle + Menu Button) */}
          <div className="md:hidden flex items-center gap-1">
            <button
              ...
              onClick={openLanguageModal}
              title={t("nav.switchLanguage", "Change Language")}
              className="p-2 rounded-xl text-gray-700 dark:text-slate-200 hover:bg-gray-100 dark:hover:bg-slate-800 transition-colors min-h-[40px] min-w-[40px] flex items-center justify-center cursor-pointer font-bold text-xs"
            >
              <Globe className="w-4 h-4 text-[#0F4C81] dark:text-sky-400 mr-1" />
              <span>{currentLanguage.badge}</span>
            </button>
            <ThemeToggle variant="icon" />
            <button
      ...
      {menuOpen && (
        <div className="fixed inset-0 top-16 z-40 md:hidden flex flex-col">
          ...
          <div className="relative bg-white dark:bg-slate-900 border-b border-gray-200 dark:border-slate-800 px-5 pt-3 pb-6 space-y-2 shadow-2xl overflow-y-auto max-h-[calc(100dvh-4rem)]">
            ...
            {/* Language Preference Row (Mobile) */}
            <div className="py-2.5 px-3 border-t border-gray-100 dark:border-slate-800 flex items-center justify-between">
              <span className="text-xs font-semibold text-gray-600 dark:text-slate-400">{t("nav.language", "Language")}</span>
              <button
                type="button"
                onClick={() => {
                  setMenuOpen(false);
                ...
                className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-gray-100 dark:bg-slate-800 text-xs font-bold text-gray-800 dark:text-slate-200 hover:border-[#0F4C81] cursor-pointer"
              >
                <Globe className="w-3.5 h-3.5 text-[#0F4C81] dark:text-sky-400" />
                <span>{currentLanguage.nativeName} ({currentLanguage.badge})</span>
              </button>
            </div>

            {/* Appearance Theme Row (Mobile) */}
            <div className="py-2.5 px-3 border-t border-gray-100 dark:border-slate-800 flex items-center justify-between">
              <span className="text-xs font-semibold text-gray-600 dark:text-slate-400">{t("nav.themePreference", "Theme Preference")}</span>
            ...
            <div className="pt-2 border-t border-gray-100 dark:border-slate-800 flex flex-col gap-2.5">
              {isAuthenticated && user ? (
                <>
                  ...
                  >
                    <LayoutDashboard className="w-4 h-4" />
                    {t("nav.myDashboard", "Go to Dashboard")}
                  </Link>
                  <button
                    onClick={() => {
                      logout();
            ...
            <div className="pt-4 mt-2 border-t border-gray-100 dark:border-slate-800 flex flex-col gap-2">
              <span className="text-[11px] font-bold text-gray-400 dark:text-slate-500 uppercase tracking-wider">{t("nav.directSupport", "Direct Support")}</span>
