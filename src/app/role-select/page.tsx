    ...
    <div className="min-h-screen bg-gradient-to-br from-[#F9FAFB] to-blue-50/40 dark:from-[#0B1120] dark:to-[#0F172A] flex items-center justify-center py-12 px-4 sm:px-6 transition-colors">
      <div className="max-w-4xl w-full mx-auto">
        ...
        <div className="text-center mb-10 sm:mb-12">
          ...
          <h1 className="font-display text-2xl sm:text-3xl lg:text-4xl font-bold text-gray-900 dark:text-white mb-2">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-5 sm:gap-6 mb-8">
          ...
          <button
            ...
            className={`group relative p-6 sm:p-8 rounded-3xl border-2 text-left transition-all duration-300 cursor-pointer min-h-[44px] ${
              hovered === "student"
                ? "border-[#0F4C81] bg-[#0F4C81] shadow-xl shadow-blue-500/20 text-white"
                : "border-gray-200 dark:border-slate-800 bg-white dark:bg-slate-900/90 hover:border-[#0F4C81]/60 shadow-card text-gray-900 dark:text-white"
            }`}
          >
            <div
              className={`w-14 h-14 sm:w-16 sm:h-16 rounded-2xl flex items-center justify-center mb-5 sm:mb-6 transition-colors ${
                hovered === "student" ? "bg-white/20" : "bg-[#0F4C81]/10 dark:bg-[#0F4C81]/30"
              }`}
            >
              <GraduationCap className={`w-7 h-7 sm:w-8 sm:h-8 ${hovered === "student" ? "text-white" : "text-[#0F4C81] dark:text-blue-400"}`} />
            </div>
            <h2
              className={`font-display text-xl sm:text-2xl font-bold mb-1.5 ${
                hovered === "student" ? "text-white" : "text-gray-900 dark:text-white"
              }`}
            >
              I&apos;m a Student
            </h2>
            <p className={`text-xs sm:text-sm mb-5 leading-relaxed ${hovered === "student" ? "text-white/80" : "text-gray-600 dark:text-slate-400"}`}>
              ...
              {[
                ...
                <li
                  ...
                  className={`flex items-center gap-2 text-xs ${
                    hovered === "student" ? "text-white/90" : "text-gray-700 dark:text-slate-300"
                  }`}
                >
                  <CheckCircle
                    className={`w-3.5 h-3.5 shrink-0 ${
                      hovered === "student" ? "text-[#FF6B35]" : "text-[#0F4C81] dark:text-blue-400"
                    }`}
                  />
                  <span>{f}</span>
                </li>
          ...
          <button
            ...
            className={`group relative p-6 sm:p-8 rounded-3xl border-2 text-left transition-all duration-300 cursor-pointer min-h-[44px] ${
              hovered === "owner"
                ? "border-[#FF6B35] bg-[#FF6B35] shadow-xl shadow-orange-500/20 text-white"
                : "border-gray-200 dark:border-slate-800 bg-white dark:bg-slate-900/90 hover:border-[#FF6B35]/60 shadow-card text-gray-900 dark:text-white"
            }`}
          >
            <div
              className={`w-14 h-14 sm:w-16 sm:h-16 rounded-2xl flex items-center justify-center mb-5 sm:mb-6 transition-colors ${
                hovered === "owner" ? "bg-white/20" : "bg-[#FF6B35]/10 dark:bg-[#FF6B35]/30"
              }`}
            >
              <Building2 className={`w-7 h-7 sm:w-8 sm:h-8 ${hovered === "owner" ? "text-white" : "text-[#FF6B35] dark:text-orange-400"}`} />
            </div>
            <h2
              className={`font-display text-xl sm:text-2xl font-bold mb-1.5 ${
                hovered === "owner" ? "text-white" : "text-gray-900 dark:text-white"
              }`}
            >
              I&apos;m an Owner
            </h2>
            <p className={`text-xs sm:text-sm mb-5 leading-relaxed ${hovered === "owner" ? "text-white/80" : "text-gray-600 dark:text-slate-400"}`}>
            ...
            <ul className="space-y-2 mb-6">
              {[
                ...
                <li
                  key={f}
                  className={`flex items-center gap-2 text-xs ${
                    hovered === "owner" ? "text-white/90" : "text-gray-700 dark:text-slate-300"
                  }`}
                >
                  <CheckCircle
                    className={`w-3.5 h-3.5 shrink-0 ${
                      hovered === "owner" ? "text-white" : "text-[#FF6B35] dark:text-orange-400"
                    }`}
                  />
                  <span>{f}</span>
                </li>
        ...
        <p className="text-center text-gray-600 dark:text-slate-400 text-xs sm:text-sm">
          Already have an account?{" "}
          <Link href="/login" className="text-[#0F4C81] dark:text-blue-400 font-bold hover:underline">
            Sign In here
          </Link>
        </p>
      </div>
