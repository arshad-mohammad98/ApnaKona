...
export default function OwnerDashboard() {
  ...
  const ownerListings = listings.filter((l) => l.ownerId === OWNER_ID);
      ...
      <div className="min-h-screen bg-[#F9FAFB] flex items-center justify-center p-4">
        <div className="text-center max-w-sm bg-white p-8 rounded-3xl border border-gray-100 shadow-card">
          ...
          <p className="text-gray-500 text-xs sm:text-sm mb-6">
            Sign in as an accommodation owner or master administrator to access the management portal.
          </p>
          <Link
  ...
  const TABS = [
    ...
    { key: "inquiries", label: "Inquiries", icon: MessageCircle, count: DUMMY_INQUIRIES.length },
    { key: "reviews", label: "Reviews", icon: Star, count: ownerReviews.length },
    { key: "complaints", label: "Grievances", icon: AlertCircle, count: ownerComplaints.length },
  ];
    ...
    <div className="min-h-screen bg-[#F9FAFB] pb-16">
          ...
          <div className="max-w-7xl mx-auto flex flex-wrap items-center justify-between gap-3">
            <div className="flex items-center gap-2.5">
              ...
              <div className="text-xs">
                ...
                <span className="text-slate-300">
                  Viewing Owner Management Portal with master permissions.
                </span>
              </div>
            </div>
            <div className="flex items-center gap-2">
              <Link
                ...
                className="px-3 py-1.5 bg-[#0F4C81] hover:bg-[#0d3f6e] text-white rounded-lg text-xs font-bold transition-colors shadow-xs"
              >
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3.5 sm:gap-4 mb-8">
          {stats.map(({ icon: Icon, label, value, change }) => (
            <div
              key={label}
              className="bg-white dark:bg-slate-800/90 rounded-3xl p-4 sm:p-5 shadow-card border border-gray-100 dark:border-slate-700/80 flex flex-col justify-between"
            >
              <div className="flex items-center justify-between mb-3">
                <div className="w-9 h-9 bg-[#FF6B35]/10 dark:bg-orange-500/20 rounded-xl flex items-center justify-center">
                  <Icon className="w-4 h-4 text-[#FF6B35] dark:text-orange-400" />
                </div>
                <span className="text-[10px] sm:text-xs font-bold text-emerald-800 dark:text-emerald-300 bg-emerald-100 dark:bg-emerald-950/60 px-2 py-0.5 rounded-full">
                  {change}
                </span>
              </div>
              <div>
          ...
          {TABS.map(({ key, label, icon: Icon, count }) => (
            <button
              ...
              className={`flex-1 min-w-[120px] flex items-center justify-center gap-2 py-2.5 px-3 rounded-xl text-xs sm:text-sm font-semibold transition-all duration-200 ease-out cursor-pointer min-h-[44px] ${
                activeTab === key
                  ? "bg-[#FF6B35] text-white shadow-sm scale-[1.02]"
        ...
        {/* Tab: Listings */}
        {activeTab === "listings" && (
          <div key="listings" className="space-y-4 tab-pane-transition">
            ...
            {ownerListings.map((l) => (
              <div
                key={l.id}
                className="bg-white dark:bg-[#131D31] rounded-3xl p-4 sm:p-5 shadow-card border border-gray-100 dark:border-slate-800 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4"
              >
                ...
                <div className="flex items-center gap-2 w-full sm:w-auto">
                  <Link
                    href={`/hostels/${l.id}`}
                    className="flex-1 sm:flex-initial px-4 py-2 bg-gray-100 hover:bg-gray-200 dark:bg-slate-800 dark:hover:bg-slate-700 text-gray-800 dark:text-slate-200 rounded-xl text-xs font-semibold text-center transition-colors min-h-[38px] flex items-center justify-center"
                  >
                    Preview
                  </Link>
                  <button
                    onClick={() => removeListing(l.id)}
                    className="p-2 text-red-500 hover:bg-red-50 dark:hover:bg-red-950/40 rounded-xl transition-colors cursor-pointer min-h-[38px] min-w-[38px] flex items-center justify-center"
        ...
        {/* Tab: Inquiries */}
        {activeTab === "inquiries" && (
          <div key="inquiries" className="space-y-4 tab-pane-transition">
            ...
            {DUMMY_INQUIRIES.map((i) => (
              <div
                key={i.id}
                className="bg-white dark:bg-[#131D31] rounded-3xl p-5 shadow-card border border-gray-100 dark:border-slate-800 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4"
              >
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-2 mb-1.5">
                    <span className="font-bold text-sm text-gray-900 dark:text-white">{i.student}</span>
                    <span
                    ...
                    <span className="text-[11px] text-gray-400 dark:text-slate-500 ml-auto sm:ml-0">{i.date}</span>
                  </div>
                  <p className="text-xs font-semibold text-[#0F4C81] dark:text-sky-400 mb-1">Re: {i.listing}</p>
                  <p className="text-xs sm:text-sm text-gray-600 dark:text-slate-300 leading-relaxed">{i.message}</p>
                </div>
                <button className="w-full sm:w-auto px-4 py-2.5 bg-[#0F4C81] hover:bg-[#0d3f6e] dark:bg-blue-600 dark:hover:bg-blue-500 text-white rounded-xl text-xs font-semibold cursor-pointer shrink-0 min-h-[40px]">
                  Reply to Student
                </button>
              </div>
            ))}
        ...
        {/* Tab: Reviews */}
        {activeTab === "reviews" && (
          <div key="reviews" className="space-y-4 tab-pane-transition">
            <h2 className="font-display font-bold text-base sm:text-lg text-gray-900 dark:text-white mb-2">
              Student Reviews ({ownerReviews.length})
            </h2>
            {ownerReviews.map((r) => (
              <div key={r.id} className="bg-white dark:bg-[#131D31] rounded-3xl p-5 shadow-card border border-gray-100 dark:border-slate-800">
                <div className="flex items-center justify-between mb-2">
                ...
                <p className="text-xs sm:text-sm text-gray-600 dark:text-slate-300 leading-relaxed">{r.comment}</p>
                <span className="text-[11px] text-gray-400 dark:text-slate-500 block mt-2">{r.date}</span>
              </div>
            ))}
          </div>
        )}
          ...
          <div key="complaints" className="space-y-4 tab-pane-transition">
            {ownerComplaints.length > 0 ? (
              ownerComplaints.map((c) => (
                <div key={c.id} className="bg-white dark:bg-[#131D31] rounded-3xl p-5 shadow-card border border-red-100 dark:border-red-900/40 space-y-3">
                  <div className="flex items-start justify-between gap-3">
                    <p className="font-bold text-sm text-gray-900 dark:text-white">{c.subject}</p>
                    <Badge variant="warning">{c.status}</Badge>
                  </div>
                  <p className="text-xs text-gray-500 dark:text-slate-400">Filed by: {c.userName} • {c.createdAt}</p>
              ...
              <div className="text-center py-16 bg-white dark:bg-[#131D31] rounded-3xl border border-gray-100 dark:border-slate-800 p-8 shadow-card">
                ...
                <p className="text-sm font-semibold text-gray-800 dark:text-white">No complaints against your properties 🎉</p>
                <p className="text-xs text-gray-400 dark:text-slate-400 mt-1">Students love staying at your accommodations.</p>
              </div>
            )}
          </div>
        )}
      ...
      {addListingOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-xs p-4">
          <div className="bg-white dark:bg-[#131D31] rounded-3xl p-5 sm:p-7 w-full max-w-2xl max-h-[90dvh] overflow-y-auto shadow-2xl border border-gray-100 dark:border-slate-800 animate-fade-up">
            <div className="flex items-center justify-between mb-6 pb-3 border-b border-gray-100 dark:border-slate-800">
              ...
              <button
                ...
                className="p-1.5 text-gray-400 hover:text-gray-700 dark:hover:text-slate-200 rounded-xl hover:bg-gray-100 dark:hover:bg-slate-800 cursor-pointer min-h-[40px] min-w-[40px] flex items-center justify-center"
              >
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            >
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3.5">
                {[
                  ...
                  <div key={label}>
                    <label className="block text-xs sm:text-sm font-semibold text-gray-700 dark:text-slate-300 mb-1">{label}</label>
                    <input
                      type="text"
                      placeholder={ph}
                      required
                      className="w-full px-3.5 py-2.5 border border-gray-200 dark:border-slate-700 bg-white dark:bg-slate-900 text-gray-900 dark:text-white placeholder-gray-400 dark:placeholder-slate-500 rounded-xl text-xs sm:text-sm outline-none focus:border-[#FF6B35] min-h-[44px]"
                    />
                  </div>
                ))}
                {[
                ...
                {[
                  ...
                  <div key={label as string}>
                    ...
                    <select className="w-full px-3.5 py-2.5 border border-gray-200 dark:border-slate-700 rounded-xl text-xs sm:text-sm outline-none focus:border-[#FF6B35] bg-white dark:bg-slate-900 text-gray-900 dark:text-white min-h-[44px]">
              ...
              <div>
                <label className="block text-xs sm:text-sm font-semibold text-gray-700 dark:text-slate-300 mb-1">Property Description</label>
                <textarea
                  rows={3}
                  placeholder="Describe amenities, proximity to colleges, and special house rules..."
                  className="w-full px-3.5 py-2.5 border border-gray-200 dark:border-slate-700 bg-white dark:bg-slate-900 text-gray-900 dark:text-white placeholder-gray-400 dark:placeholder-slate-500 rounded-xl text-xs sm:text-sm outline-none focus:border-[#FF6B35] resize-none"
                />
              </div>
              ...
              <div className="flex flex-wrap items-center gap-3 pt-1">
                {[
                  ...
                  <label key={item} className="flex items-center gap-2 text-xs sm:text-sm text-gray-700 dark:text-slate-300 cursor-pointer">
                    <input type="checkbox" className="w-4 h-4 rounded border-gray-300 dark:border-slate-600 accent-[#FF6B35]" />
                    <span>{item}</span>
                  </label>
                ))}
              </div>
              ...
              <div className="flex flex-col sm:flex-row gap-3 pt-3">
                <button
                  type="button"
                  onClick={() => setAddListingOpen(false)}
                  className="w-full sm:w-1/2 py-3 border border-gray-200 dark:border-slate-700 text-gray-700 dark:text-slate-300 rounded-xl text-xs sm:text-sm font-semibold hover:bg-gray-50 dark:hover:bg-slate-800 min-h-[44px] cursor-pointer"
                >
                  Cancel
                </button>
                <button
