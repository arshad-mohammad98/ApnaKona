...
function ImageGallery({ images, title }: { images: string[]; title: string }) {
  ...
  return (
    <>
      ...
      {/* Thumbnails Row: Horizontally Scrollable without page overflow */}
        ...
        {images.map((img, i) => (
          <button
            ...
            className={`h-16 w-24 rounded-xl overflow-hidden border-2 shrink-0 transition-all cursor-pointer ${
              i === current ? "border-[#0F4C81] ring-2 ring-[#0F4C81]/20 scale-105" : "border-transparent opacity-70 hover:opacity-100"
            }`}
          >
    ...
    <div className="min-h-screen bg-[#F9FAFB] pb-24 lg:pb-12 pt-4 sm:pt-6">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex items-center justify-between gap-3 text-xs sm:text-sm text-gray-500 mb-5">
          <div className="flex items-center gap-1.5 overflow-hidden text-ellipsis whitespace-nowrap">
            <Link href="/" className="hover:text-[#0F4C81] shrink-0">Home</Link>
            <span>/</span>
            <Link href="/search" className="hover:text-[#0F4C81] shrink-0">Search</Link>
            <span>/</span>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 lg:gap-8">
          {/* Main Details Column (2 Cols on Desktop) */}
          <div className="lg:col-span-2 space-y-6 sm:space-y-8">
            ...
            <div className="bg-white rounded-3xl p-5 sm:p-7 shadow-card border border-gray-100">
              ...
              <div className="rounded-2xl overflow-hidden h-56 sm:h-72 bg-gray-100 border border-gray-200">
                <iframe
              ...
              <p className="flex items-center gap-1.5 text-gray-500 text-xs sm:text-sm mt-3">
                <MapPin className="w-4 h-4 text-[#0F4C81] shrink-0" />
                {listing.address}
              </p>
            </div>
            ...
            <div className="bg-white rounded-3xl p-5 sm:p-7 shadow-card border border-gray-100">
              <div className="flex items-center justify-between mb-6">
                <h2 className="font-display font-semibold text-base sm:text-lg text-[#1A1A2E]">
                  Verified Student Reviews ({listing.reviewCount})
                </h2>
                <div className="flex items-center gap-1">
                  <Star className="w-4 h-4 sm:w-5 sm:h-5 text-amber-500 fill-amber-500" />
                  <span className="font-bold text-base sm:text-lg text-gray-900">{listing.rating}</span>
              ...
              {reviews.length > 0 ? (
                ...
                <div className="text-center py-8 text-gray-400">
                  ...
                  <p className="text-sm">No reviews yet for this listing. Be the first to leave one!</p>
                </div>
              )}
            </div>
          </div>

          {/* Owner Sidebar (Sticky on Desktop, In-flow on Tablet/Mobile) */}
          <div className="space-y-6">
            <div className="bg-white rounded-3xl p-6 shadow-card border border-gray-100 lg:sticky lg:top-24">
              <h2 className="font-display font-semibold text-base text-[#1A1A2E] mb-4">
              ...
              <div className="flex items-center gap-3.5 mb-5 pb-4 border-b border-gray-100">
                {listing.ownerAvatar ? (
                  ...
                  <div className="w-13 h-13 rounded-2xl bg-[#0F4C81] flex items-center justify-center text-white font-bold text-lg shrink-0">
                ...
                <div className="min-w-0">
                  ...
                  <p className="text-xs text-green-600 flex items-center gap-1 font-medium mt-0.5">
                    <CheckCircle className="w-3.5 h-3.5 shrink-0" /> Verified Partner
                  </p>
                </div>
              </div>
              ...
              <div className="space-y-3">
                ...
                <button
                  ...
                  className="w-full flex items-center justify-center gap-2 py-3 px-4 bg-[#0F4C81] hover:bg-[#0d3f6e] text-white rounded-xl text-sm font-bold transition-colors min-h-[44px] cursor-pointer shadow-sm"
                >
              ...
              <div className="mt-5 pt-4 border-t border-gray-100 text-[11px] text-gray-400 space-y-1">
                <p>Listing ID: {listing.id}</p>
                <p>Posted: {listing.postedAt} • Direct Owner Connect</p>
              </div>
            </div>
          </div>
        </div>
            ...
            {chatSent ? (
              ...
              <>
                ...
                <textarea
                  ...
                  rows={4}
                  placeholder="Hi, I'm interested in booking a visit for this room..."
                ...
                <button
                  ...
                  className="w-full mt-3 py-3 bg-[#0F4C81] hover:bg-[#0d3f6e] text-white rounded-xl text-sm font-bold transition-colors min-h-[46px] cursor-pointer"
                >
      ...
      {reportOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-xs p-4">
          <div className="bg-white rounded-3xl p-6 w-full max-w-md shadow-2xl animate-fade-up">
            <div className="flex items-center justify-between mb-4">
              <h3 className="font-display font-bold text-base text-red-600">Report Listing</h3>
              <button
            ...
            <p className="text-gray-500 text-xs sm:text-sm mb-4">
              Help us maintain authentic student housing. What seems wrong with this listing?
            </p>
            <select className="w-full border border-gray-200 rounded-xl px-3.5 py-2.5 text-xs sm:text-sm mb-3 outline-none focus:border-red-400 min-h-[44px] bg-white">
              <option>Photos don&apos;t match reality</option>
              <option>Incorrect pricing or hidden charges</option>
              <option>Room already occupied / unavailable</option>
              <option>Fraudulent owner credentials</option>
            ...
            <textarea
              ...
              className="w-full border border-gray-200 rounded-xl p-3.5 text-xs sm:text-sm outline-none focus:border-red-400 resize-none mb-4"
            />
            <button
              onClick={() => setReportOpen(false)}
              className="w-full py-3 bg-red-500 hover:bg-red-600 text-white rounded-xl text-sm font-bold transition-colors min-h-[46px] cursor-pointer"
            >
              Submit Report to ApnaKona Safety Team
            </button>
          </div>
        </div>
      )}
