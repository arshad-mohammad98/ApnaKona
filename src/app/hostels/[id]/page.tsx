      ...
      >
        ...
        {onToggleSave && (
          <button
            ...
            title={isSaved ? "Remove from saved properties" : "Save property to favorites"}
            aria-label={isSaved ? "Remove from saved properties" : "Save property to favorites"}
            className={`absolute top-3 left-3 w-10 h-10 rounded-full flex items-center justify-center backdrop-blur-md transition-all z-10 cursor-pointer shadow-md ${
              isSaved
                ? "bg-red-500 text-white scale-105 ring-2 ring-white/60"
                : "bg-black/40 text-white hover:bg-white hover:text-red-500 hover:scale-105"
            }`}
          >
            <Heart className={`w-5 h-5 transition-transform active:scale-125 ${isSaved ? "fill-white text-white" : ""}`} />
          </button>
        )}
...
export default function HostelDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    setIsSaved(isListingSavedLocally(listing.id));
  }, [listing.id]);
  ...
  const handleToggleSave = async () => {
    ...
    setIsSaved(next);
    if (next) {
      setSaveToast("Saved to your profile & Supabase!");
      await saveListingToSupabase(listing, user?.id);
    } else {
      setSaveToast("Removed from saved properties");
      await removeSavedListingFromSupabase(listing.id, user?.id);
    }
    ...
    <div className="min-h-screen bg-[#F9FAFB] pb-24 lg:pb-12 pt-4 sm:pt-6">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex items-center justify-between gap-3 text-xs sm:text-sm text-gray-500 mb-5">
          ...
          <div className="flex items-center gap-2 shrink-0">
            <button
              ...
              title={isSaved ? "Saved to your profile" : "Save property"}
              className={`flex items-center gap-1.5 px-3 py-1.5 rounded-xl border text-xs font-semibold cursor-pointer shrink-0 min-h-[36px] transition-colors ${
                isSaved
                  ? "bg-red-50 border-red-200 text-red-600"
                  : "border-gray-200 bg-white hover:border-red-300 text-gray-700"
              }`}
            >
              <Heart className={`w-3.5 h-3.5 ${isSaved ? "fill-red-500 text-red-500" : "text-gray-500"}`} />
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 lg:gap-8">
          ...
          <div className="lg:col-span-2 space-y-6 sm:space-y-8">
            {/* Gallery */}
            <div className="bg-white dark:bg-slate-800/90 rounded-3xl p-4 sm:p-6 shadow-card border border-gray-100 dark:border-slate-700/80">
              <ImageGallery images={listing.images} title={listing.title} isSaved={isSaved} onToggleSave={handleToggleSave} />
            </div>

            {/* Title & Overview Card */}
            <div className="bg-white dark:bg-slate-800/90 rounded-3xl p-5 sm:p-7 shadow-card border border-gray-100 dark:border-slate-700/80">
              <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4 pb-6 border-b border-gray-100 dark:border-slate-700/60">
                <div>
                  ...
                  <h1 className="font-display text-xl sm:text-2xl lg:text-3xl font-bold text-gray-900 dark:text-white leading-tight">
                    {listing.title}
                  </h1>
                  <p className="flex items-center gap-1.5 text-gray-600 dark:text-slate-300 text-xs sm:text-sm mt-2">
                    <MapPin className="w-4 h-4 text-[#0F4C81] dark:text-sky-400 shrink-0" />
                    {listing.address}
                  </p>
                </div>

                <div className="sm:text-right shrink-0 bg-blue-50/50 dark:bg-slate-700/40 sm:bg-transparent p-4 sm:p-0 rounded-2xl">
                  <p className="text-2xl sm:text-3xl font-bold text-[#0F4C81] dark:text-sky-400">
                    ₹{listing.price.toLocaleString("en-IN")}
                    <span className="text-xs sm:text-sm font-normal text-gray-400 dark:text-slate-400">/mo</span>
                  </p>
                  ...
                  <div className="flex items-center sm:justify-end gap-1 mt-1.5">
                    ...
                    <span className="font-bold text-sm text-gray-900 dark:text-amber-200">{listing.rating}</span>
                    <span className="text-gray-400 dark:text-slate-400 text-xs">({listing.reviewCount} reviews)</span>
                  </div>
                </div>
              </div>

              <div className="pt-5">
                <h2 className="font-display font-semibold text-sm sm:text-base text-gray-900 dark:text-white mb-2">
                  About this property
                </h2>
                <p className="text-gray-600 dark:text-slate-300 text-xs sm:text-sm leading-relaxed whitespace-pre-line">
                  {listing.description}
                </p>
              </div>
            </div>
            ...
            <div className="bg-white dark:bg-slate-800/90 rounded-3xl p-5 sm:p-7 shadow-card border border-gray-100 dark:border-slate-700/80">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3">
                {listing.amenities.map((a) => (
                  <div key={a} className="flex items-center gap-3 p-3 bg-gray-50 dark:bg-slate-700/70 rounded-2xl">
                    <div className="w-9 h-9 bg-[#0F4C81]/10 dark:bg-sky-500/20 rounded-xl flex items-center justify-center text-[#0F4C81] dark:text-sky-400 shrink-0">
                      {AMENITY_ICONS[a] || <CheckCircle className="w-4 h-4" />}
                    </div>
                    <span className="text-xs sm:text-sm font-medium text-gray-800 dark:text-slate-200">{a}</span>
                  </div>
                ))}
                {listing.hasMess && (
            ...
            {/* House Rules */}
            <div className="bg-white dark:bg-[#131D31] rounded-3xl p-5 sm:p-7 shadow-card border border-gray-100 dark:border-slate-800">
              <h2 className="font-display font-semibold text-base sm:text-lg text-gray-900 dark:text-white mb-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3.5">
                {[
                  ...
                  <div key={label} className="flex items-center gap-3.5 p-3.5 bg-gray-50 dark:bg-slate-800/80 rounded-2xl">
                    <div
                      className={`w-9 h-9 rounded-xl flex items-center justify-center shrink-0 ${
                        ok ? "bg-green-100 dark:bg-green-950/60 text-green-700 dark:text-green-300" : "bg-red-100 dark:bg-red-950/60 text-red-600 dark:text-red-300"
                      }`}
                    >
                    ...
                    <div>
                      ...
                      <p className="text-xs sm:text-sm font-semibold text-gray-800 dark:text-slate-100">{value}</p>
                    </div>
                  </div>
                ))}
              </div>
            ...
            {/* Map Section */}
            <div className="bg-white dark:bg-[#131D31] rounded-3xl p-5 sm:p-7 shadow-card border border-gray-100 dark:border-slate-800">
              <h2 className="font-display font-semibold text-base sm:text-lg text-gray-900 dark:text-white mb-3">
              ...
              <div className="rounded-2xl overflow-hidden h-72 sm:h-96 border border-gray-100 dark:border-slate-800 bg-gray-100 dark:bg-slate-800 relative">
                <GoogleMapView
                  lat={listing.lat}
                  lng={listing.lng}
                  zoom={15}
            ...
            <div className="bg-white dark:bg-[#131D31] rounded-3xl p-5 sm:p-7 shadow-card border border-gray-100 dark:border-slate-800">
              <div className="flex items-center justify-between mb-6">
                <div>
                  <h2 className="font-display font-semibold text-base sm:text-lg text-gray-900 dark:text-white">
                    Student Reviews
                  </h2>
                ...
                <div className="flex items-center gap-1.5 bg-amber-50 dark:bg-amber-950/40 px-3 py-1.5 rounded-xl">
                  <Star className="w-5 h-5 text-amber-500 fill-amber-500" />
                  <span className="font-bold text-base text-gray-900 dark:text-white">{listing.rating}</span>
                  <span className="text-xs text-gray-400 dark:text-slate-400">/ 5.0</span>
                </div>
              </div>
            ...
            <div className="bg-white rounded-3xl p-5 sm:p-7 shadow-card border border-gray-100 sticky top-24 space-y-6">
              ...
              <div className="pb-4 border-b border-gray-100 flex items-start justify-between">
                ...
                <button
                  type="button"
                  onClick={handleToggleSave}
                  title={isSaved ? "Saved to your profile" : "Save property to favorites"}
                  className={`p-3 rounded-2xl border transition-all cursor-pointer shadow-xs ${
                    isSaved
                      ? "bg-red-50 border-red-200 text-red-500 hover:bg-red-100"
                      : "bg-gray-50 border-gray-200 text-gray-400 hover:text-red-500 hover:border-red-200 hover:bg-white"
                  }`}
                >
                  <Heart className={`w-5 h-5 transition-transform active:scale-125 ${isSaved ? "fill-red-500 text-red-500" : ""}`} />
                </button>
              </div>
              ...
              <div className="space-y-3">
                ...
                <button
                  type="button"
                  onClick={handleToggleSave}
                  className={`w-full flex items-center justify-center gap-2 py-3 px-4 rounded-xl text-sm font-semibold transition-all min-h-[44px] cursor-pointer border ${
                    isSaved
                      ? "bg-red-50 border-red-200 text-red-600 hover:bg-red-100 shadow-xs"
                      : "bg-white border-gray-200 text-gray-700 hover:border-red-300 hover:bg-red-50/40 hover:text-red-600"
                  }`}
                >
                  <Heart className={`w-4 h-4 ${isSaved ? "fill-red-500 text-red-500" : "text-gray-500"}`} />
                  <span>{isSaved ? "✓ Saved to Profile & Supabase" : "Save Property to Wishlist"}</span>
                </button>
                <button
      ...
      <div className="lg:hidden fixed bottom-0 left-0 right-0 p-3.5 bg-white/95 backdrop-blur-md border-t border-gray-200 shadow-2xl flex items-center gap-3 z-30">
        ...
        <button
          type="button"
          onClick={handleToggleSave}
          className={`flex items-center justify-center p-3 border rounded-xl min-h-[46px] min-w-[46px] transition-colors cursor-pointer ${
            isSaved ? "bg-red-50 border-red-300 text-red-500" : "border-gray-200 text-gray-600 hover:border-red-300"
          }`}
      ...
      {saveToast && (
        <div className="fixed bottom-20 lg:bottom-6 right-6 z-50 flex items-center gap-2.5 px-4 py-3 bg-gray-900 text-white text-xs sm:text-sm font-medium rounded-2xl shadow-2xl border border-gray-700 animate-fade-up">
          <Heart className={`w-4 h-4 ${isSaved ? "text-red-400 fill-red-400" : "text-gray-400"}`} />
          <span>{saveToast}</span>
        </div>
      )}
    </div>
